...
class DefaultSource extends StreamSourceProvider
  ...
  // THIS SHORT NAME USED for readStream() method's FORMAT argument
  ...
  private case class ConfigUpdater(module: String, kafkaParams: Map[String, String]) {
    ...
    def set(key: String, value: String): this.type = {
      map.put(key, value)
      log.info(s"$module: Set $key to $value, earlier value: ${kafkaParams.get(key).getOrElse("")}")
      this
    }
    ...
    def setIfUnset(key: String, value: String): ConfigUpdater = {
      if (!map.containsKey(key)) {
        map.put(key, value)
        log.info(s"$module: Set $key to $value")
      }
      this
    }
  ...
  // sink to kafka, use producer api
  override def createSink(sqlContext: SQLContext,
                          parameters: Map[String, String],
                          partitionColumns: Seq[String],
                          outputMode: OutputMode): Sink = {
    val defaultTopic = parameters.get(TOPICS).map(_.trim)
    new KafkaSink(sqlContext, new ju.HashMap[String, Object](parameters.asJava), defaultTopic)
  }
}
