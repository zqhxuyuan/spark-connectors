...
/** A leaking bucket rate limiter.
  ...
  *
  * https://github.com/datastax/spark-cassandra-connector/blob/master/spark-cassandra-connector/src/main/scala/com/datastax/spark/connector/writer/RateLimiter.scala
  */
class RateLimit(
                   rate: Long,
                   bucketSize: Long,
  ...
  val bucketFill = new AtomicLong(0L)
  val lastTime = new AtomicLong(time()) //Avoid a large initial step
